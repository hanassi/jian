...
cat() {
    ...
    for file in "${files[@]}"; do
        if [ -e "$file" ]; then
            command cat "${args[@]}" "$file" >> $CREATE_FILE 2>&1
        else
            echo "[ERROR] 파일이 존재하지 않습니다: $file" >> $CREATE_FILE 2>&1
        fi
    done
}
...
ls() {
    ...
    for target in "${targets[@]}"; do
        if [ -e "$target" ]; then
            command ls "${args[@]}" "$target" >> $CREATE_FILE 2>&1
        else
            echo "[ERROR] 경로가 존재하지 않습니다: $target" >> $CREATE_FILE 2>&1
        fi
    done
}
...
grep() {
    ...
    for file in "${files[@]}"; do
        if [ -e "$file" ]; then
            command grep "${args[@]}" "$pattern" "$file" >> $CREATE_FILE 2>&1
        else
            echo "[ERROR] 파일이 존재하지 않습니다: $file" >> $CREATE_FILE 2>&1
        fi
    done
